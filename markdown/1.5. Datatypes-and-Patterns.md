...
데이터 모델링 관점에서, 귀납 데이터타입은 다른 언어에서 봉인된(sealed) 추상 클래스를 사용할 수 있는 많은 동일한 컨텍스트에서 사용됩니다.
...
하지만 이러한 표현의 세부 사항은 상당히 다릅니다. 특히, 각 비추상 클래스는 새로운 타입과 새로운 데이터 할당 방법을 모두 생성합니다. 객체 지향 예제에서, `True`와 `False`는 모두 `Bool`보다 더 구체적인 타입이지만, Lean 정의는 새로운 타입 `Bool`만 도입합니다.
...
위의 `Bool` 예제와 마찬가지로, 이것은 Lean 동치보다 더 많은 타입을 정의합니다.
추가적으로, 이 예제는 Lean의 데이터타입 생성자가 C# 또는 Java의 생성자보다 훨씬 더 추상 클래스의 하위 클래스처럼 보인다는 것을 강조합니다. 여기서 보여진 생성자는 실행할 초기화 코드를 포함하기 때문입니다.
...
합 타입은 또한 TypeScript에서 판별된 합(discriminated unions)을 인코딩하기 위해 문자열 태그를 사용하는 것과 유사합니다.
...
C# 및 Java와 마찬가지로, 이 인코딩은 Lean보다 더 많은 타입으로 끝나갑니다. `Zero`와 `Succ`이 각각 그 자체로 타입이기 때문입니다.
또한 Lean의 생성자가 JavaScript 또는 TypeScript의 객체에 해당하며 내용을 식별하는 태그를 포함한다는 것을 보여줍니다.

## 1.5.1. 패턴 매칭[🔗](find/?domain=Verso.Genre.Manual.section&name=pattern-matching "Permalink")
...
## 1.5.2. 재귀 함수[🔗](find/?domain=Verso.Genre.Manual.section&name=recursive-functions "Permalink")
