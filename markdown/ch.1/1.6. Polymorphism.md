...
두 메시지 모두 누락된 암시적 인수를 설명하기 위해 *동일한* 메타변수를 사용하는데, 이는 실제 해결책의 값을 결정할 수는 없었지만 두 누락된 부분이 해결책을 공유할 것이라고 Lean이 결정했다는 것을 의미합니다.
...
다음 섹션에서 설명하는 [동시 매칭(Simultaneous matching)](Getting-to-Know-Lean/Additional-Conveniences/#simultaneous-matching)은 종종 더 우아한 문제 해결 방법입니다.
