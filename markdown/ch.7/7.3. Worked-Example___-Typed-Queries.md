...
[product type에 대한 초기 섹션](Getting-to-Know-Lean/Polymorphism/#prod)에서 설명했듯이, Lean의 product type과 tuple은 right-associative입니다.
이는 nested pair가 ordinary flat tuple과 동등함을 의미합니다.
...
projection이 의미를 가지려면, 더 작은 schema는 더 큰 schema의 subschema여야 하며, 이는 더 작은 schema의 모든 column이 더 큰 schema에 존재해야 함을 의미합니다.
...
이 relation은 reflexive이며, 이는 모든 schema가 자신의 subschema임을 의미합니다.
...
query의 결과는 table입니다. 이는 query language의 각 operation이 table과 함께 작동하는 해당 implementation을 필요로 함을 의미합니다.
