...
`repeat` 루프는 `for` 루프처럼 `break` 및 `continue`를 지원합니다. [implementation of `feline`](Hello___-World___/Worked-Example___--cat/#streams)의 `dump` 함수는 재귀 함수를 사용하여 영원히 실행합니다:
