...
이 datatype은 `List`과 매우 유사합니다. 차이점은 `List.cons`가 리스트의 나머지를 저장하는 반면, `more`은 요청에 따라 남은 값을 계산해야 하는 함수를 저장한다는 것입니다. 이는 `Many`의 소비자가 일정 수의 결과를 찾았을 때 검색을 중지할 수 있음을 의미합니다.
