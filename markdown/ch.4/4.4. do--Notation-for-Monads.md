...
[Hello, World!](Hello___-World___/#hello-world)에서 `do` 구문은 `IO` 액션을 결합하는 데 사용되지만 이러한 프로그램의 의미는 직접 설명됩니다. Monad를 사용하여 프로그래밍하는 방법을 이해하는 것은 이제 `do`가 기본 Monad 연산자의 사용으로 어떻게 변환되는지에 따라 설명될 수 있음을 의미합니다.
