...
이는 `Nat → Nat`이 `Type`이고, `Type → Type`이 `Type 1`이며, `Type 3`이 `Type 1 → Type 2`임을 의미합니다.
...
특히, 이는 ordinary type을 가지는 값들에 대한 predicate들이 `Prop`에 있음을 의미합니다.
...
이는 그것이 실제 type들을 포함하는 데 사용될 수 없음을 의미합니다. 왜냐하면 그 argument가 `Type 1`인 `Type`이 되기 때문입니다:
...
이는 `Prop`이 `Nat`과 같은 이유로 `List`에 제공할 적절한 argument라는 것을 의미합니다.
