...
`Validate`는 입력이 수용 가능한 여러 방법이 있는 상황에서도 사용할 수 있습니다.
입력 형식 `RawInput`의 경우, 레거시 시스템 규칙을 구현하는 대안적 비즈니스 규칙은 다음과 같을 수 있습니다:
...
1. 모든 인간 사용자는 4자리 출생 연도를 제공해야 합니다.
2. 1970년 이전에 태어난 사용자는 오래된 기록이 불완전하므로 이름을 제공할 필요가 없습니다.
3. 1970년 이후에 태어난 사용자는 이름을 제공해야 합니다.
4. 회사는 출생 연도로 `"FIRM"`을 입력하고 회사 이름을 제공해야 합니다.
...
1970년에 태어난 사용자를 위한 특별한 규정은 없습니다.
그들은 포기하거나, 출생 연도에 대해 거짓말하거나, 전화를 걸 것으로 예상됩니다.
회사는 이를 허용할 수 있는 비즈니스 비용으로 간주합니다.

The following inductive type captures the values that can be produced from these stated rules:

다음 귀납 타입은 이러한 명시된 규칙으로부터 생성될 수 있는 값들을 나타냅니다:
...
이러한 규칙에 대한 검증자는 더 복잡합니다. 세 가지 경우를 모두 처리해야 하기 때문입니다.
중첩된 `if` 표현식의 시리즈로 작성할 수 있지만, 세 가지 경우를 독립적으로 설계한 후 결합하는 것이 더 쉽습니다.
이를 위해서는 오류 메시지를 보존하면서 실패에서 복구하는 방법이 필요합니다:
...
실패 복구의 이 패턴은 충분히 일반적이어서 Lean은 `OrElse`라는 타입 클래스에 부착된 내장 문법을 가지고 있습니다:

`class OrElse (α : Type) where
orElse : α → (Unit → α) → α`

표현식 `E1 <|> E2`는 `OrElse.orElse E1 (fun () => E2)`의 단축형입니다.
`Validate`에 대한 `OrElse`의 인스턴스는 이 문법을 오류 복구에 사용할 수 있게 합니다:
...
`LegacyCheckedInput`의 검증자는 각 생성자에 대한 검증자로부터 구성할 수 있습니다.
회사에 대한 규칙에 따르면 출생 연도는 문자열 `"FIRM"`이어야 하고 이름은 비어있지 않아야 합니다.
그러나 생성자 `LegacyCheckedInput.company`는 출생 연도를 전혀 나타내지 않으므로 `<*>`를 사용하여 이를 쉽게 수행할 수 있는 방법이 없습니다.
핵심은 인수를 무시하는 함수를 `<*>`와 함께 사용하는 것입니다.
...
부울 조건이 참인지 확인하되 이 사실에 대한 증거를 타입에 기록하지 않는 것은 `checkThat`으로 수행할 수 있습니다:
...
`checkCompany`의 이 정의는 `checkThat`을 사용한 후 결과 `Unit` 값을 버립니다:
...
그러나 이 정의는 상당히 지저분합니다.
두 가지 방법으로 단순화할 수 있습니다.
첫 번째는 첫 번째 `<*>` 사용을 `*>`라고 불리는 특수화된 버전으로 대체하는 것입니다. 이는 첫 번째 인수로부터 반환된 값을 자동으로 무시합니다.
이 연산자는 `SeqRight`라는 타입 클래스로도 제어되며, `E1 *> E2`는 `SeqRight.seqRight E1 (fun () => E2)`에 대한 문법 설탕입니다:
...
`seqRight`의 기본 구현은 `seq` 측면에서 제공됩니다: `seqRight (a : f α) (b : Unit → f β) : f β := pure (fun _ x => x) <*> a <*> b ()`.

`seqRight`를 사용하면 `checkCompany`가 더 간단해집니다:
...
또 다른 단순화가 가능합니다.
모든 `Applicative`에 대해 `pure f <*> E`는 `f <$> E`와 동등합니다.
즉, `pure`를 사용하여 `Applicative` 타입에 배치된 함수를 적용하는 `seq`를 사용하는 것은 과도하며, 함수는 단순히 `Functor.map`을 사용하여 적용될 수 있습니다.
이 단순화는 다음을 산출합니다:
...
`LegacyCheckedInput`의 남은 두 생성자는 필드에 서브타입을 사용합니다.
서브타입을 검사하기 위한 일반적인 도구는 이들을 읽기 쉽게 만들 것입니다:
...
함수의 인수 목록에서 타입 클래스 `[Decidable (p v)]`가 인수 `v`와 `p`의 지정 후에 나타나는 것이 중요합니다.
그렇지 않으면 수동으로 제공된 값이 아닌 자동 암시 인수의 추가 집합을 참조할 것입니다.
`Decidable` 인스턴스는 명제 `p v`를 `if`를 사용하여 검사할 수 있게 해주는 것입니다.
...
세 가지 경우의 검증자는 `<|>`를 사용하여 결합할 수 있습니다:
...
성공한 경우들은 예상대로 `LegacyCheckedInput`의 생성자를 반환합니다:
...
많은 타입은 실패와 복구의 개념을 지원합니다.
[다양한 모나드에서 산술 표현식 평가](Monads/Example___-Arithmetic-in-Monads/#nondeterministic-search) 섹션의 `Many` 모나드는 `Option`과 마찬가지로 그러한 타입 중 하나입니다.
둘 다 이유를 제공하지 않고 실패를 지원합니다 (`Except`와 `Validate`와 달리, 이들은 무엇이 잘못되었는지에 대한 표시를 요구합니다).
...
`Alternative` 클래스는 실패와 복구를 위한 추가 연산자를 가진 Applicative 펑터를 설명합니다:

`class Alternative (f : Type → Type) extends Applicative f where
failure : f α
orElse : f α → (Unit → f α) → f α`

`Add α` 구현자가 무료로 `HAdd α α α` 인스턴스를 얻듯이, `Alternative` 구현자는 무료로 `OrElse` 인스턴스를 얻습니다:
...
`Option`에 대한 `Alternative`의 구현은 첫 번째 `none`이 아닌 인수를 유지합니다:
...
마찬가지로 `Many`의 구현은 `Many.union`의 일반적인 구조를 따르며, 지연을 유도하는 `Unit` 매개변수가 다르게 배치되어 약간의 차이가 있습니다:
...
다른 타입 클래스처럼 `Alternative`는 `Alternative`를 구현하는 *모든* Applicative 펑터에 대해 작동하는 다양한 연산의 정의를 가능하게 합니다.
가장 중요한 것 중 하나는 `guard`이며, 이는 판정 가능한 명제가 거짓일 때 `failure`를 야기합니다:
...
모나드 프로그램에서 실행을 조기에 종료하는 것은 매우 유용합니다.
`Many`에서는 다음의 자연수의 모든 짝수 약수를 계산하는 프로그램과 같이 검색의 전체 분기를 필터링하는 데 사용할 수 있습니다:
