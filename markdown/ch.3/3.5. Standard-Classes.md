...
`And`와 `Or`는 이미 논리 결합자(logical connective)의 이름으로 사용되고 있기 때문에, `HAnd`와 `HOr`의 homogeneous 버전은 `And`와 `Or` 대신 `AndOp`과 `OrOp`이라고 불립니다.
...
사실, [polymorphism에 대한 첫 번째 섹션](Getting-to-Know-Lean/Polymorphism/#polymorphism)의 `Firewood` 정의에서 `deriving Repr` 구문은 instance deriving의 예시입니다.
...
인자를 무시하는 함수들은 항상 같은 값을 반환하기 때문에 *constant function*이라고 불립니다.
