...
*Applicative functor*는 두 가지 추가 연산을 사용할 수 있는 functor입니다: `pure`와 `seq`.
`pure`는 `Monad`에서 사용되는 동일한 연산자이며, `Monad`는 실제로 `Applicative`에서 상속받습니다.
`seq`는 `map`과 유사하며, 함수를 사용하여 datatype의 내용을 변환할 수 있도록 해줍니다.
그러나 `seq`에서는 함수 자체가 datatype에 포함됩니다: `f (α → β) → (Unit → f α) → f β`.
함수가 타입 `f` 아래에 있으면 `Applicative` 인스턴스가 함수를 어떻게 적용할지를 제어할 수 있지만, `Functor.map`은 함수를 무조건 적용합니다.
두 번째 인수는 `Unit →`로 시작하는 타입을 가지며, 이는 함수가 절대 적용되지 않을 경우 `seq` 정의가 short-circuit되도록 합니다.
...
이러한 short-circuiting 동작의 가치는 `Applicative Option`의 인스턴스에서 볼 수 있습니다:
...
동일한 고려사항이 `Except`에 대한 `Applicative` 인스턴스를 알립니다:
...
이러한 short-circuiting 동작은 함수 자체가 아니라 함수를 *둘러싼* `Option` 또는 `Except` 구조에만 의존합니다.
...
Monad는 순차적으로 명령문을 실행하는 개념을 순수 함수형 언어로 캡처하는 방법으로 볼 수 있습니다.
한 명령문의 결과는 어떤 추가 명령문이 실행될지에 영향을 줄 수 있습니다.
이는 `bind`의 타입에서 볼 수 있습니다: `m α → (α → m β) → m β`.
첫 번째 명령문의 결과 값은 다음에 실행할 명령문을 계산하는 함수의 입력입니다.
`bind`의 연속적인 사용은 명령형 프로그래밍 언어의 명령문 시퀀스와 같으며, `bind`는 조건문 및 루프와 같은 제어 구조를 구현할 수 있을 만큼 강력합니다.
...
이 유추를 따르면, `Applicative`는 side effect가 있는 언어에서 함수 적용을 캡처합니다.
Kotlin이나 C#과 같은 언어에서 함수의 인수는 왼쪽에서 오른쪽으로 평가됩니다.
이전 인수에 의해 수행되는 side effect는 이후 인수에 의해 수행되는 side effect보다 먼저 발생합니다.
그러나 함수는 인수의 특정 *값*에 의존하는 커스텀 short-circuiting 연산자를 구현할 만큼 강력하지 않습니다.
...
대신 연산자 `<*>`가 사용됩니다.
이 연산자는 두 번째 인수를 `fun () => ...`로 감싸므로 호출 사이트가 단순화됩니다.
다시 말해, `E1 <*> E2`는 `Seq.seq E1 (fun () => E2)`의 syntactic sugar입니다.

`seq`를 여러 인수와 함께 사용할 수 있게 하는 핵심 특성은 여러 인수를 가진 Lean 함수가 실제로는 나머지 인수를 기다리는 다른 함수를 반환하는 단일 인수 함수라는 것입니다.
즉, `seq`의 첫 번째 인수가 여러 인수를 기다리고 있다면, `seq`의 결과도 나머지를 기다리고 있을 것입니다.
예를 들어, `some Plus.plus`는 `Option (Nat → Nat → Nat)` 타입을 가질 수 있습니다.
한 개의 인수를 제공하면, `some Plus.plus <*> some 4`는 `Option (Nat → Nat)` 타입을 결과로 합니다.
이는 `seq`와 함께 다시 사용될 수 있으므로, `some Plus.plus <*> some 4 <*> some 7`은 `Option Nat` 타입을 가집니다.
...
모든 functor가 applicative인 것은 아닙니다.
`Pair`는 내장 product type인 `Prod`와 유사합니다:

`structure Pair (α β : Type) : Type where
first : α
second : β`

`Except`와 마찬가지로, `Pair`는 `Type → Type → Type` 타입을 가집니다.
이는 `Pair α`가 `Type → Type` 타입을 가지며, `Functor` 인스턴스가 가능함을 의미합니다:
...
확인해야 할 두 가지 속성은 `id <$> Pair.mk x y` = `Pair.mk x y`이고 `f <$> g <$> Pair.mk x y` = `(f ∘ g) <$> Pair.mk x y`라는 것입니다.
첫 번째 속성은 왼쪽의 평가를 단계별로 살펴보고 오른쪽으로 평가됨을 확인함으로써 검증할 수 있습니다:

`id <$> Pair.mk x y` = `Pair.mk x (id y)` = `Pair.mk x y`

두 번째는 양쪽을 단계별로 살펴보고 동일한 결과를 산출함을 확인함으로써 검증할 수 있습니다:
...
그러나 `Applicative` 인스턴스를 정의하려고 시도하면 잘 작동하지 않습니다.
`pure`의 정의가 필요합니다:
...
범위 내에 `β` 타입의 값이 있으며 (즉, `x`), 언더스코어의 오류 메시지는 다음 단계가 생성자 `Pair.mk`를 사용하는 것임을 제안합니다:
...
`pure`는 `Applicative (Pair α)`의 인스턴스를 정의하기 위해 *모든 가능한 타입* `α`에 대해 작동해야 하므로 이는 불가능합니다.
결국 호출자가 `α`를 값이 없는 `Empty`로 선택할 수 있기 때문입니다.
...
폼에서 사용자 입력을 검증할 때, 일반적으로 한 번에 많은 오류를 제공하는 것이 한 번에 하나의 오류를 제공하는 것보다 낫다고 간주됩니다.
이를 통해 사용자는 컴퓨터를 만족시키기 위해 필요한 것을 한눈에 볼 수 있으며, 오류를 필드별로 수정할 때 괴로움을 받지 않습니다.
...
이상적으로, 사용자 입력의 검증은 검증을 수행하는 함수의 타입에서 보여야 합니다.
예를 들어, 텍스트 상자에 숫자가 포함되어 있는지 확인하는 것은 실제 숫자 타입을 반환해야 합니다.
유효성 검사 루틴은 입력이 유효성 검사를 통과하지 못할 때 예외를 발생시킬 수 있습니다.
그러나 예외에는 심각한 단점이 있습니다: 예외는 첫 번째 오류에서 프로그램을 종료하므로 오류 목록을 누적하는 것이 불가능합니다.
...
한편, 오류 목록을 누적한 다음 비어 있지 않을 때 실패하는 일반적인 설계 패턴도 문제가 있습니다.
입력 데이터의 각 부분을 검증하는 긴 중첩 `if` 명령문의 시퀀스는 유지하기 어렵고, 오류 메시지 하나 또는 둘을 놓치기 쉽습니다.
이상적으로, 새로운 값을 반환할 수 있으면서도 자동으로 오류 메시지를 추적하고 누적하는 API를 사용하여 유효성 검사를 수행할 수 있습니다.
...
`Validate`라는 applicative functor는 이러한 스타일의 API를 구현하는 한 가지 방법을 제공합니다.
`Except` monad와 마찬가지로 `Validate`는 검증된 데이터를 정확하게 특성화하는 새로운 값을 구성할 수 있습니다.
`Except`와 달리 `Validate`는 목록이 비어 있는지 확인하는 것을 잊을 위험 없이 여러 오류를 누적할 수 있습니다.
...
1. 이름은 비어 있으면 안 됩니다
2. 출생 연도는 숫자이고 음이 아니어야 합니다
3. 출생 연도는 1900보다 크고 폼이 검증되는 연도보다 작거나 같아야 합니다
...
이것들을 datatype으로 표현하려면 *subtype*이라고 하는 새로운 기능이 필요합니다.
이 도구를 손에 들고 applicative functor를 사용하여 오류를 추적하는 유효성 검사 프레임워크를 작성할 수 있으며, 이러한 규칙을 프레임워크에서 구현할 수 있습니다.
...
이러한 조건을 표현하는 것은 `Subtype`이라고 불리는 하나의 추가 Lean 타입으로 가장 쉽습니다:

`structure Subtype {α : Type} (p : α → Prop) where
val : α
property : p val`

이 구조는 두 개의 타입 매개변수를 가집니다: 데이터의 타입 `α`인 암시적 매개변수와 `α`에 대한 predicate인 명시적 매개변수 `p`.
...
*Predicate*는 변수를 포함하는 논리 명령문으로, 조회를 위해 인덱스가 범위 내에 있다는 것이 무엇을 의미하는지 설명하는 [`GetElem`의 매개변수](Overloading-and-Type-Classes/Arrays-and-Indexing/#overloading-indexing)와 같은 값으로 대체하여 실제 명령문을 산출할 수 있습니다.
`Subtype`의 경우, predicate은 predicate이 성립하는 `α`의 값의 일부 부분집합을 슬라이싱합니다.
구조의 두 필드는 각각 `α`의 값과 값이 predicate `p`를 만족한다는 증거입니다.
Lean은 `Subtype`에 대한 특별한 구문을 가집니다.
`p`가 `α → Prop` 타입을 가지면, 타입 `Subtype p`를 `{x : α // p x}`로 쓸 수도 있고, 심지어 타입 `α`를 자동으로 추론할 수 있을 때 `{x // p x}`로도 쓸 수 있습니다.
...
[Positive numbers를 inductive types으로 표현하는 것](Overloading-and-Type-Classes/Positive-Numbers/#positive-numbers)은 명확하고 프로그래밍하기 쉽습니다.
그러나 이는 주요 단점이 있습니다.
`Nat`과 `Int`는 Lean 프로그램의 관점에서 ordinary inductive types의 구조를 가지지만, 컴파일러는 이들을 특별히 취급하고 빠른 arbitrary-precision number 라이브러리를 사용하여 구현합니다.
이는 추가적인 사용자 정의 타입의 경우에는 해당하지 않습니다.
그러나 `Nat`의 subtype이 이를 0이 아닌 숫자로 제한하면 새로운 타입이 효율적인 표현을 사용하면서도 컴파일 시간에 0을 제외할 수 있습니다:
...
가장 작은 fast positive number는 여전히 1입니다.
이제 inductive type의 생성자가 아니라 각도 괄호로 구성된 구조의 인스턴스입니다.
...
명제 `1 > 0`은 decidable이므로 `decide` tactic은 필요한 증거를 생성합니다.
`OfNat` 인스턴스는 `Pos`의 경우와 매우 유사하지만, `n + 1 > 0`임을 증명하는 짧은 tactic proof를 사용합니다:

`instance : OfNat FastPos (n + 1) where
ofNat := ⟨n + 1, byn:Nat⊢ n + 1 > 0 simpAll goals completed! 🐙⟩`

여기서, `decide`가 구체적인 값을 요구하지만 해당 명제가 `n + 1 > 0`이므로 `simp`가 필요합니다.
...
Subtype은 양날의 칼입니다.
유효성 검사 규칙의 효율적인 표현을 허용하지만, 이러한 규칙을 유지하는 부담을 라이브러리의 사용자에게 옮겨지는데, 사용자는 중요한 불변식을 위반하지 않고 있음을 *증명*해야 합니다.
일반적으로 라이브러리 내부에서 이를 사용하고 사용자에게 모든 불변식이 만족되도록 자동으로 보장하는 API를 제공하는 것이 좋으며, 필요한 모든 증명은 라이브러리 내부에 있어야 합니다.
...
타입 `α`의 값이 subtype `{x : α // p x}`에 있는지 확인하는 것은 일반적으로 명제 `p x`가 decidable이어야 합니다.
[equality and ordering classes에 대한 섹션](Overloading-and-Type-Classes/Standard-Classes/#equality-and-ordering)은 decidable propositions이 `if`와 함께 어떻게 사용될 수 있는지 설명합니다.
`if`가 decidable proposition과 함께 사용될 때, 이름을 제공할 수 있습니다.
`then` 분기에서, 이름은 명제가 참이라는 증거로 바인딩되고, `else` 분기에서는 명제가 거짓이라는 증거로 바인딩됩니다.
...
`then` 분기에서, `h`는 `n > 0`이라는 증거로 바인딩되고, 이 증거는 `Subtype`의 생성자의 두 번째 인수로 사용될 수 있습니다.
...
* 출생 연도는 `String`이 아닌 `Nat`으로 표현됩니다
* Subtype은 이름 및 출생 연도 필드의 허용된 값을 제한하는 데 사용됩니다
...
입력 검증기는 현재 연도와 `RawInput`을 인수로 취하고, 검증된 입력 또는 최소 하나의 검증 실패를 반환해야 합니다.
...
함수를 매핑하면 `Except`에 대한 `Functor` 인스턴스와 마찬가지로 존재할 수 있는 모든 성공적인 값을 변환합니다:
...
`Validate`에 대한 `Applicative` 인스턴스는 `Except`의 인스턴스와 중요한 차이가 있습니다: `Except`의 인스턴스는 발생한 첫 번째 오류에서 종료되지만, `Validate`의 인스턴스는 함수와 인수 분기 *모두*에서 모든 오류를 누적하도록 주의합니다:
...
`reportError`와 같은 helper는 코드를 더 읽기 쉽게 만듭니다.
이 애플리케이션에서 오류 보고서는 필드 이름과 쌍을 이룬 메시지로 구성됩니다:
...
`Validate`에 대한 `Applicative` 인스턴스는 각 필드의 검사 절차를 독립적으로 작성하고 구성할 수 있도록 합니다.
이름을 확인하는 것은 문자열이 비어 있지 않음을 보장한 다음 이 사실을 `Subtype` 형태의 증거로 반환하는 것으로 구성됩니다.
...
`then` 분기에서 `h`는 `name = ""`라는 증거로 바인딩되고, `else` 분기에서는 `¬name = ""`라는 증거로 바인딩됩니다.
...
분명히 일부 검증 오류는 다른 검사를 불가능하게 만듭니다.
예를 들어, 혼동된 사용자가 숫자 대신 `"syzygy"`라는 단어를 작성했다면 출생 연도 필드가 1900보다 큰지 확인하는 것은 의미가 없습니다.
숫자의 허용 범위를 확인하는 것은 필드에 실제로 숫자가 포함되어 있음을 확인한 후에만 의미가 있습니다.
...
이 함수의 타입 시그니처는 `Monad` 인스턴스의 `bind`로 사용하기에 적합하지만, 그렇게 하지 않을 좋은 이유가 있습니다.
이것들은 [`Applicative` 계약을 설명하는 섹션](Functors___-Applicative-Functors___-and-Monads/The-Applicative-Contract/#additional-stipulations)에서 설명됩니다.
...
출생 연도가 숫자인지 확인하기 위해 `String.toNat? : String → Option Nat`이라고 하는 내장 함수가 유용합니다.
가장 사용자 친화적인 방법은 먼저 `String.trim`을 사용하여 선행 및 후행 공백을 제거하는 것입니다:
...
`checkInput` 테스트는 실제로 여러 피드백을 반환할 수 있음을 보여줍니다:
...
`checkInput`을 사용한 폼 검증은 `Applicative`가 `Monad`에 비해 갖는 주요 장점을 보여줍니다.
`>>=`는 첫 번째 단계의 값에 따라 프로그램 실행의 나머지 부분을 수정할 수 있는 충분한 힘을 제공하므로, 첫 번째 단계에서 값을 받아 *반드시* 전달해야 합니다.
값을 받지 못하면 (예: 오류가 발생했으므로) `>>=`는 프로그램의 나머지 부분을 실행할 수 없습니다.
`Validate`는 여전히 프로그램의 나머지 부분을 실행하는 것이 유용한 이유를 보여줍니다: 초기 데이터가 필요하지 않은 경우, 프로그램의 나머지 부분을 실행하면 유용한 정보를 얻을 수 있습니다 (이 경우 더 많은 검증 오류).
`Applicative`의 `<*>`는 결과를 재결합하기 전에 두 인수를 모두 실행할 수 있습니다.
마찬가지로 `>>=`는 순차 실행을 강제합니다.
각 단계는 다음 단계가 실행되기 전에 완료되어야 합니다.
이것은 일반적으로 유용하지만, 프로그램의 실제 데이터 종속성에서 자연스럽게 나타나는 서로 다른 스레드의 병렬 실행을 불가능하게 합니다.
`Monad`와 같은 더 강력한 추상화는 API 소비자가 사용할 수 있는 유연성을 증가시키지만, API 구현자가 사용할 수 있는 유연성을 감소시킵니다.